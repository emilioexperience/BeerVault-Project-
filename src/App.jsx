...
import { Camera, Star, MapPin, DollarSign, TrendingUp, Users, Search, Filter, Heart, MessageCircle, Share2, Award, BarChart3, Map, Plus, X, ChevronDown, Menu, Home, Book, Globe, User, Settings, Upload, Check, Trophy, LogOut, Eye, EyeOff, Image } from 'lucide-react';
...
export default function BeerTrackerApp() {
  ...
  // Initialize app - load from localStorage or use demo data
  useEffect(() => {
    const initApp = () => {
      // Load users
  ...
  // Save beers to localStorage whenever they change
  useEffect(() => {
    if (beers.length > 0) {
  ...
  // Update userBeers when beers or currentUser changes
  useEffect(() => {
    if (currentUser) {
  ...
  const handleAddBeer = (newBeer) => {
    ...
    // Update user's weekly beer count
    ...
    <div className="bg-white rounded-2xl shadow-xl overflow-hidden border-2 border-amber-200 hover:shadow-2xl transition-all duration-500">
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="flex items-center justify-between pt-4 border-t-2 border-amber-100">
          ...
          <button className="flex items-center gap-2 px-4 py-2 rounded-lg bg-amber-100 text-amber-900 font-medium hover:bg-amber-200 transition-all duration-300">
...
function JournalView({ userBeers, currentUser }) {
  ...
  const stats = {
    ...
    averageRating: userBeers.length > 0
      ? (userBeers.reduce((sum, b) => sum + b.rating, 0) / userBeers.length).toFixed(1)
      : 0,
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition-all ${
                view === 'grid' ? 'bg-gradient-to-r from-amber-500 to-orange-500 text-white' : 'bg-amber-100 text-amber-900'
              }`}
            >
              Grid
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-lg font-medium transition-all ${
                view === 'list' ? 'bg-gradient-to-r from-amber-500 to-orange-500 text-white' : 'bg-amber-100 text-amber-900'
              }`}
            >
              List
            </button>
            ...
            <div key={beer.id} className="bg-white rounded-2xl shadow-xl overflow-hidden border-2 border-amber-200 hover:shadow-2xl transition-all duration-500">
              <div className="h-40 bg-gradient-to-br from-amber-100 to-orange-100 flex items-center justify-center overflow-hidden">
                {beer.image ? (
                  ...
                  <div className="text-6xl">{beer.imageEmoji || '🍺'}</div>
                )}
              </div>
              <div className="p-4">
                ...
                <div className="flex items-center gap-1 mb-2">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} size={16} className={i < beer.rating ? 'fill-amber-500 text-amber-500' : 'text-amber-300'} />
                  ))}
                </div>
            ...
            <div key={beer.id} className="bg-white rounded-xl shadow-lg p-4 border-2 border-amber-200 flex items-center gap-4">
              <div className="w-16 h-16 bg-gradient-to-br from-amber-100 to-orange-100 rounded-lg flex items-center justify-center overflow-hidden">
                {beer.image ? (
                  ...
                  <div className="text-3xl">{beer.imageEmoji || '🍺'}</div>
                )}
              </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg p-6 border-2 border-amber-200">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-4 rounded-xl border-2 border-amber-300 focus:ring-2 focus:ring-amber-500 bg-amber-50 text-amber-900"
          />
        </div>
      </div>
          ...
          <div key={beer.id} className="bg-white rounded-2xl shadow-xl overflow-hidden border-2 border-amber-200 hover:shadow-2xl transition-all">
            <div className="flex">
              ...
              <div className="flex-1 p-4">
                ...
                <div className="flex items-center gap-1 mb-2">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} size={14} className={i < beer.rating ? 'fill-amber-500 text-amber-500' : 'text-amber-300'} />
                  ))}
                  <span className="text-sm font-bold text-amber-900 ml-1">{beer.rating}</span>
                </div>
                <button className="w-full mt-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white py-2 rounded-lg font-semibold hover:shadow-lg transition-all">
                  Add to Want List
                </button>
              </div>
            </div>
...
function MapView({ beers, currentUser }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Map placeholder - in production you'd use Google Maps or Mapbox */}
      <div className="bg-gradient-to-br from-blue-100 to-blue-200 rounded-2xl shadow-lg border-2 border-blue-300 h-96 flex items-center justify-center relative overflow-hidden">
        <div className="absolute inset-0 opacity-20">
          <div className="absolute top-1/4 left-1/4 w-4 h-4 bg-amber-500 rounded-full animate-ping"></div>
          <div className="absolute top-1/3 right-1/3 w-4 h-4 bg-amber-500 rounded-full animate-ping" style={{ animationDelay: '0.5s' }}></div>
          <div className="absolute bottom-1/3 left-1/2 w-4 h-4 bg-amber-500 rounded-full animate-ping" style={{ animationDelay: '1s' }}></div>
        </div>
        <div className="text-center z-10">
          <div className="text-6xl mb-4">🗺️</div>
          <p className="text-blue-800 font-bold text-xl">Interactive Map</p>
          <p className="text-blue-600 text-sm">Showing {Object.keys(locations).length} locations</p>
        </div>
      </div>

      {/* Location List */}
      <div className="bg-white rounded-2xl shadow-lg p-6 border-2 border-amber-200">
        <h3 className="text-xl font-bold text-amber-900 mb-4">Your Beer Locations</h3>
          ...
          <div className="space-y-4">
            {Object.values(locations).map((loc, idx) => (
              <div key={idx} className="flex items-center justify-between p-4 bg-amber-50 rounded-xl border-2 border-amber-200">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-gradient-to-br from-amber-400 to-orange-500 rounded-full flex items-center justify-center text-2xl shadow-md">
                    📍
                  </div>
                  <div>
                ...
                <div className="flex -space-x-2">
                  {loc.beers.slice(0, 3).map((beer, i) => (
                    <div key={i} className="w-8 h-8 bg-amber-200 rounded-full border-2 border-white flex items-center justify-center text-sm">
                      {beer.imageEmoji || '🍺'}
                    </div>
                  ))}
                  {loc.beers.length > 3 && (
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold mb-1" style={{ fontFamily: '"Playfair Display", serif' }}>
              <Trophy className="inline mr-2" size={28} />
              Weekly Leaderboard
            </h2>
      ...
      <div className="bg-white rounded-2xl shadow-lg border-2 border-amber-200 overflow-hidden">
        <div className="p-4 bg-amber-50 border-b-2 border-amber-200">
          <h3 className="font-bold text-amber-900">Full Rankings</h3>
        </div>
          ...
          {sortedUsers.map((user, idx) => (
            <div
              ...
              className={`flex items-center justify-between p-4 ${
                user.id === currentUser.id ? 'bg-amber-50' : ''
              }`}
            >
              <div className="flex items-center gap-4">
                <span className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${
                  idx < 3 ? 'bg-gradient-to-r from-amber-400 to-orange-400 text-white' : 'bg-gray-200 text-gray-600'
                }`}>
                  {idx + 1}
                </span>
                <div className="w-10 h-10 bg-gradient-to-br from-amber-200 to-orange-200 rounded-full flex items-center justify-center text-xl">
                  {user.avatar}
                </div>
                <div>
                  <p className={`font-bold ${user.id === currentUser.id ? 'text-amber-600' : 'text-amber-900'}`}>
                    {user.username}
                    {user.id === currentUser.id && <span className="ml-2 text-xs bg-amber-200 px-2 py-1 rounded-full">You</span>}
                  </p>
...
function AnalyticsView({ userBeers, currentUser }) {
  const styleBreakdown = userBeers.reduce((acc, beer) => {
    acc[beer.style] = (acc[beer.style] || 0) + 1;
    return acc;
  }, {});

  const sortedStyles = Object.entries(styleBreakdown)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5);
                ...
                <div key={style}>
                  ...
                  <div className="h-3 bg-amber-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${(count / totalBeers) * 100}%` }}
                    />
                  </div>
                </div>
              ))}
...
function AddBeerModal({ onClose, onSubmit, flavorOptions, beerStyles }) {
  ...
  // Simulated location suggestions (in production, use Google Places API)
  const mockLocations = [
      ...
      <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border-4 border-amber-300">
        <div className="sticky top-0 bg-gradient-to-r from-amber-900 via-orange-800 to-amber-900 text-white p-6 rounded-t-3xl border-b-4 border-amber-700 z-10">
          <div className="flex items-center justify-between">
            ...
            <button onClick={onClose} className="w-10 h-10 bg-white bg-opacity-20 rounded-full flex items-center justify-center hover:bg-opacity-30">
              <X size={24} />
            </button>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              <div
                ...
                className="w-32 h-32 bg-gradient-to-br from-amber-100 to-orange-100 rounded-xl border-2 border-dashed border-amber-300 flex items-center justify-center cursor-pointer hover:border-amber-500 transition-all overflow-hidden"
              >
                {imagePreview ? (
                  <img src={imagePreview} alt="Preview" className="w-full h-full object-cover" />
                ) : (
                  <div className="text-center">
                    <Image size={32} className="mx-auto text-amber-400 mb-1" />
                ...
                <div className="flex gap-2 flex-wrap">
                  {['🍺', '🍻', '🥃', '🍷', '🥂'].map(emoji => (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        setFormData({ ...formData, imageEmoji: emoji, image: null });
                        setImagePreview(null);
                      }}
                      className={`text-3xl p-2 rounded-lg transition-all ${
                        formData.imageEmoji === emoji && !formData.image
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-xl border-2 border-amber-300 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-amber-50"
              placeholder="Start typing: Riga, Labietis..."
            />
              ...
              <div className="absolute z-10 w-full mt-1 bg-white rounded-xl shadow-xl border-2 border-amber-200 overflow-hidden">
                {locationSuggestions.map((loc, idx) => (
                  <button
                    ...
                    className="w-full px-4 py-3 text-left hover:bg-amber-50 flex items-center gap-2 border-b border-amber-100 last:border-0"
                  >
                    <MapPin size={16} className="text-amber-500" />
                    <span className="text-amber-900">{loc.name}</span>
                  </button>
                ))}
              </div>
            )}
          ...
          <div>
            ...
            <textarea
              ...
              rows="4"
              className="w-full px-4 py-3 rounded-xl border-2 border-amber-300 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-amber-50 resize-none"
          ...
          <button
            type="submit"
            className="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white py-4 rounded-xl font-bold text-lg shadow-xl hover:shadow-2xl transform hover:scale-105 transition-all"
          >
            Add Beer to Journal
          </button>
        </form>
